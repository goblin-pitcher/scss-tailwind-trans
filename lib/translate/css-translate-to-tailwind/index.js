const { formatCssProp, waterfallExec } = require("../../common");
...
const fuzzyAnalysis = (pair, options) => {
  ...
  return waterfallExec(funcArr, checkSuccess)(...pair, options);
};
...
const cssTranslateToTailwind = (decl, options) => {
  ...
  const rtn = getTransResult(getCssPropPairs(decl), options);
  rtn.text = text;
