...
const { formatCssProp, getCssPairs, formatNumRule } = require("../../common");
...
const transOriginPercent = (value) => {
  ...
  const transArr = splitValue.map((val, index) => {
    ...
    if (!index) {
      ...
      if (["0px", "0%", "0"].includes(val)) return "top";
      if (val === "100%") return "bottom";
    }
    return val;
  });
...
const findTwVariable = (rem) => (key, value) => {
  ...
  return (
    Object.keys(searchObj).find((key) => {
      ...
      return (val === value) || (formatNumRule(val) === formatNumRule(value));
    }) || null
  );
};
