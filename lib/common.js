...
const getCssPairs = (prop) => {
  const childProps = splitCssProp(prop);
  return childProps.map((p) =>
    p
      .replace(/;$/, "")
      .split(":")
      .map((item) => item.trim())
...
const formatNumRule = (str) => {
  if (!/[0-9]/.test(str)) return str;
  return str.replace(/[0-9]*\.?[0-9]*/g, (val) => {
    if (!val) return val;
